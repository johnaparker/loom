...
impl Config {
    ...
    /// Checks current working directory for .loom.toml first, then falls back to repo root.
    ...
    ///
    /// Priority: LOOM_CACHE_DIR env var > config file > default (~/.cache/loom)
    pub fn cache_dir(&self) -> Result<PathBuf> {
        // Environment variable takes precedence
        if let Ok(env_dir) = std::env::var("LOOM_CACHE_DIR") {
    ...
    pub fn check_migration_warnings(&self) -> Vec<String> {
            ...
            && let Some(home) = dirs::home_dir()
        {
            ...
            {
                warnings.push(
                    "Found worktrees at ~/worktrees but loom now defaults to ~/.worktrees. \
...
pub struct ResolvedLinearConfig {
    ...
    /// Auto-update status on loom new/merge
